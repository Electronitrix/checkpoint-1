...
class AmityRoomAllocationFunctionalityTestSuite(unittest.TestCase):
    ...
    def test_print_person_identifier(self):
        ...
        with capture(main.print_person_id, "erika","dike") as output:
            self.assertEqual(
                appOutput, output,
    ...
    def test_print_invalid_person_identifier(self):
        main.add_person("erika", "dike", "staff")
        appOutput = ("No identifier was found for the name supplied\n")
        with capture(main.print_person_id, "ina","dike") as output:
            self.assertEqual(
                appOutput, output,
                msg=("{0} != {1}").format(appOutput, output)
            )
    ...
    def test_invalid_person_identifiers_arguments_to_reallocate_person(self):
        ...
        main.add_person("erika", "dike", "staff")
        main.create_room("bench hook", 0, "office")
        appOutput = ("No person was found in memory for person identifier "
                     "supplied.\n")
        with capture(main.reallocate_person, 45, "bench hook") as output:
            self.assertEqual(
                appOutput, output,
                msg="{} != {}".format(appOutput, output)
            )
    ...
    def test_does_not_allocate_to_filled_living_space(self):
        ...
        appOutput = ("No room was found in memory with that name. You may "
    ...
    def test_print_allocations_when_there_are_allocations(self):
        ...
        appOutput.append(textwrap.fill("".join(temp), line_width))
        appOutput.append("\n\n")
    ...
    def test_print_allocations_when_there_are_no_allocations(self):
        appOutput = ("There are no allocations in memory. You can either "
    ...
    def test_print_allocations_to_file(self):
        ...
        self.assertEqual(
            appOutput, allocations,
            msg=("{0} != {1}!!!".format(appOutput, allocations))
        )
    ...
    def test_print_unallocated_to_file(self):
        ...
        appOutput = ("LIST OF ALL UNALLOCATED PEOPLE\n\n"
                     "ERIKA DIKE\n")
        self.assertEqual(
            appOutput, unallocated,
            msg=("{0} != {1}".format(appOutput, unallocated))
        )
